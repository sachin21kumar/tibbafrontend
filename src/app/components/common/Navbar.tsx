...
const Navbar = () => {
  ...
  return (
    <nav className="bg-[#141617] text-white sticky top-0 z-50 py-3">
      <div className="px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-around">
          ...
          <div className="hidden md:flex items-center space-x-6">
            {/* Static Links */}
            <a
              className="uppercase text-sm cursor-pointer tracking-wider hover:text-[#d1a054]"
            ...
            >
              <a
                className="menu_item uppercase text-sm cursor-pointer text-white tracking-wider hover:text-[#d1a054]"
                href="/"
              >
                Online Ordering
              </a>
              ...
              {desktopOpen && (
                <div
                  className="menu_item__sub_menu absolute top-full left-1/2 -translate-x-1/2
                             text-black p-6 rounded-lg shadow-xl text-white
                             grid grid-cols-2 gap-4 min-w-[300px] bg-black"
                >
            ...
            {/* Locations Dropdown */}
            <div
              className="menu_wrapper relative"
              onMouseEnter={() => setLocationsOpen(true)}
              onMouseLeave={() => setLocationsOpen(false)}
            >
              <div
                className="uppercase text-sm py-3 border-gray-700 cursor-pointer hover:text-[#d1a054]"
              ...
              {locationsOpen && (
                <div className="absolute top-full left-1/2 min-w-[300px]  -translate-x-1/2 bg-black text-white p-4 rounded-lg shadow-lg w-full grid gap-2">
                  {locations?.slice(0,3)?.map((loc: Location) => (
                    <div
                      key={loc._id}
                      className="text-sm w-full py-1 px-2 hover:text-[#d1a054] rounded cursor-pointer"
            ...
            >
              <div className="uppercase text-sm py-3 border-gray-700 cursor-pointer hover:text-[#d1a054]">
                Reservation
              </div>
              ...
              {reservationOpen && (
                <div className="absolute top-full left-1/2 -translate-x-1/2 bg-black text-white p-4 rounded-lg shadow-lg min-w-[200px]">
                  <div
                    ...
                    className="block text-sm py-2 hover:text-[#d1a054] cursor-pointer"
                  >
                    OpenTable
                  </div>
                </div>
      ...
      {/* Mobile Menu */}
      {mobileOpen && (
        <div className="md:hidden bg-[#141617] px-4 pb-4">
          {/* Static Links */}
          <div
            className="uppercase text-sm py-3 border-b border-gray-700 cursor-pointer hover:text-[#d1a054]"
            onClick={() => {
              navigate.push("/");
              setMobileOpen(false);
            }}
