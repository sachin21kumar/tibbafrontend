  ...
  return (
    <section className="py-20 bg-white text-gray-800">
      <div className="max-w-[1288px] mx-auto px-4">
        ...
        <div className="grid md:grid-cols-2 gap-x-16 gap-y-12 pt-5">
                ...
                {item.image && (
                  <>
                    ...
                    <img
                      src={item.image}
                      alt={item.name}
                      className="w-16 h-16 md:w-20 md:h-20 object-cover shadow-[inset_0_0_0.3125rem_rgba(255,255,255,0.25),_0_0.0625rem_0.1875rem_rgba(0,0,0,0.25)] rounded-[63%_37%_30%_70%/_50%_45%_55%_50%]"
                    />

                    {/* Large image on hover */}
                    <div className="absolute top-0 left-9 pb-3 -translate-y-full w-48 h-48 md:w-60 md:h-60 opacity-0 scale-75 group-hover:opacity-100 group-hover:scale-100 transition-all duration-300 ease-in-out z-50 pointer-events-none">
                      <img
                        src={item.image}
                        alt={item.name}
              ...
              <div className="flex-1">
                <div className="flex justify-between items-start mb-1">
                  <div className="flex items-center w-full justify-between gap-4">
                    <h3 className="text-[#1B2024] uppercase text-[20px] font-normal">
                      {item.name}
                    </h3>

                    <div className="flex-1  h-[2px] bg-gray-300" />

                    <span className="text-[#d1a054] text-[20px] font-medium">
                      {item.price}
                    </span>
                  </div>
                </div>
                <p className="text-base md:text-base text-[#1B2024] font-extralight opacity-[0.75] !font-[system-ui]">
                  {item.description}
                </p>
              </div>
            </div>
